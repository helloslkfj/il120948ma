<?php 
    ...
    if(isset($_POST['templatetext'])) {
        $templatetext = $_POST['templatetext'];
    ...
    if($error < 1) {
        if(isset($_SESSION["template"])) {
            ...
            $templateupdatesql = "UPDATE templates SET title=?, textt=?, datentimeinteger=?, iv=? WHERE email=? AND title=?;";
            // if we are always referencing EST time then we will always be true on when they created the template and can arrange based on recent
            ...
            executeSQL($conn, $templateupdatesql, ["s", "s", "s", "s", "s", "s"], array_merge($enctemplatedataarr, [$_SESSION["user"]->iv], [$encemail, $enctemplatetitle]), "update", 3);   
